...
from anytimeacquisition.pipelines.action_head_posterior_distill import (
    beta_mode,
    beta_nll_loss,
    canonical_aux_features,
    posterior_argmin_targets,
    run_stage,
)
...
def test_posterior_argmin_targets_shape_and_range():
    pfn, bar_dist = _tiny_pfn(x_dim=2)
    x_train, y_train = torch.rand(3, 5, 2), torch.rand(3, 5)
    target = posterior_argmin_targets(pfn, bar_dist, x_train, y_train, n_candidates=32)
    assert target.shape == (3, 2)
    assert (target >= 0.0).all() and (target <= 1.0).all()


def test_beta_mode_matches_closed_form_for_symmetric_beta():
    alpha = torch.tensor([2.0, 5.0])
    beta = torch.tensor([2.0, 5.0])
    mode = beta_mode(alpha, beta)
    assert torch.allclose(mode, torch.full_like(mode, 0.5), atol=1e-6)
...
def test_real_action_head_fits_a_single_fixed_context_better_than_blind():
    """Stage-1/3-style smoke check, kept fast: memorize one fixed context
    with the real (non-blind) pathway for a handful of steps and check loss
    drops -- the minimal signal that gradients are flowing usefully through
    cross-attention into the frozen PFN's hidden states, not just wiring."""
    torch.manual_seed(0)
    x_dim = 1
    ...
    losses = run_stage(
        ...
        blind=False, seed=0, log_every=1_000_000, fixed_context=fixed_context,
    )
    assert losses[-1] < losses[0], "beta NLL should drop when memorizing a single fixed context"


def test_blind_ablation_does_not_use_pfn_context():
    """blind=True must make the ActionHead's output identical across two
    contexts that differ only in (x_train, y_train) -- with hidden states
    zeroed, the only remaining input is the (here-identical) aux features."""
    x_dim = 1
